...
export function CreateCustomerModal({ open, onClose, tenants }: Props) {
  const createCustomer = useCreateCustomer();
  const { register, handleSubmit, reset, formState: { errors } } = useForm<CreateCustomerDto>({
    defaultValues: { email: "", password: "", firstName: "", lastName: "", tenantId: "" },
  });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-zinc-900">Create Customer</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input {...register("firstName", { required: "Required." })} className="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm outline-none focus:border-zinc-500 focus:ring-1 focus:ring-zinc-500" />
              {errors.firstName && <p className="mt-1 text-xs text-red-500">{errors.firstName.message}</p>}
            </div>
            <div>
              <label className="block text-sm font-medium text-zinc-700 mb-1">Last Name</label>
              <input {...register("lastName", { required: "Required." })} className="w-full rounded-lg border border-zinc-300 px-3 py-2 text-sm outline-none focus:border-zinc-500 focus:ring-1 focus:ring-zinc-500" />
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button type="submit" disabled={createCustomer.isPending} className="inline-flex items-center gap-1.5 rounded-md bg-zinc-900 px-4 py-2 text-sm text-white hover:bg-zinc-800 disabled:opacity-50"><Plus className="h-4 w-4" />{createCustomer.isPending ? "Creating..." : "Create"}</button>
          </div>
        </form>
      </div>
    </div>
