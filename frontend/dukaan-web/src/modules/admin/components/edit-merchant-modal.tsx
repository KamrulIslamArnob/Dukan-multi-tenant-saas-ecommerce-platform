...
import { useUpdateMerchant } from "../hooks";
import type { UpdateMerchantDto } from "../types";
...
export function EditMerchantModal({ open, onClose, userId, storeName, slug }: Props) {
  const updateMerchant = useUpdateMerchant();
  const { register, handleSubmit, reset, formState: { errors } } = useForm<UpdateMerchantDto>({
    defaultValues: { userId, storeName, slug },
  });

  useEffect(() => {
    if (open) reset({ userId, storeName, slug });
  }, [open, userId, storeName, slug, reset]);
  ...
  async function onSubmit(data: UpdateMerchantDto) {
    try {
      await updateMerchant.mutateAsync(data);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-zinc-900">Edit Merchant</h2>
