...
export function CreateTenantModal({ open, onClose }: Props) {
  ...
  const { register, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm<CreateTenantDto>({
    defaultValues: {
      storeName: "",
      slug: "",
      category: "General",
  ...
  async function onSubmit(data: CreateTenantDto) {
    try {
      await createTenant.mutateAsync(data);
      toast.success(`Tenant "${data.storeName}" created.`);
      onClose();
    } catch {
      toast.error("Failed to create tenant. Slug may already be taken.");
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-zinc-900">Create Tenant</h2>
